app.initializers.add('singleso-singleso-flarum', function() {
	...
	const disableOverride = 'singleso=0';
	...
	function controller(action, redirect) {
		...
		let url = baseURL + setting('controller') + (
			action ? '/' + action : ''
		);
	...
	function elemAction(elem) {
		switch (true) {
			...
			case (
				setting('guest') &&
				matches(elem, '.App-primaryControl *')
			): {
	...
	function disabled() {
		...
		const queryHash = ('' + window.location).replace(/[^?#]*[?#]?/, '');
	...
	window.addEventListener('click', function(e) {
		...
		const action = elemAction(e.target);
		if (!action || disabled()) {
			return;
		}
